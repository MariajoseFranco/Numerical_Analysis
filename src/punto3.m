function [K, M, P, Q, b, d, a, c, u_h, mesh, error_L2, error_Linf] = punto3(n, u_ex)
...
for i = 1:N
    for j=1:N
        % Matriz K
        K_aux1 = d1phii(j)*d1phii(i);
        K_aux2 = d2phii(j)*d1phii(i);
        K(i,j) = eval(int(K_aux1-K_aux2,x_inf,x_sup));
        % Matriz M
        M_aux1 = d1psii(j)*d1phii(i);
        M_aux2 = d2psii(j)*d1phii(i);
        M(i,j) = eval(int(M_aux1-M_aux2,x_inf,x_sup));
        % Matriz P
        P_aux1 = d1phii(j)*d1psii(i);
        P_aux2 = d2phii(j)*d1psii(i);
        P(i,j) = eval(int(P_aux1-P_aux2,x_inf,x_sup));
        % Matriz Q
        Q_aux1 = d1psii(j)*d1psii(i);
        Q_aux2 = d2psii(j)*d1psii(i);
        Q(i,j) = eval(int(Q_aux1-Q_aux2,x_inf,x_sup));
